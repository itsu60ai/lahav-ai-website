...
## 8. Decisions taken 2026-09-02: Portfolio to the STG reference, and COPY V2

### F-38 - The portfolio index and detail pages follow the STG portfolio, structurally
The client supplied STG's portfolio pages as a full HTML export plus desktop
screenshots and asked for the closest possible match. Matched: a giant
watermark heading (Hebrew, drifting as a marquee so the whole word passes
through the viewport, per F-22 no giant English), a narrow intro paragraph, a
two-column STAGGERED gallery of tall media tiles with name + tagline under
each, autoplaying muted video tiles, a full-bleed hero on the detail page, a
tag column + narrative intro, staggered desktop screenshots, a phone strip,
and a marquee "next project" band. NOT adopted: STG's light palette and
typefaces (F-19 dark theme, Heebo/Assistant stay), STG's jokey first-person
copy, and any imported imagery.

### F-39 - Detail pages differ by service kind
Client instruction: "it looks the same for every service". Each project now
carries `serviceSlug` (one of the five services, or blank). It selects the
middle of the page: web builds get browser-framed screenshots and the visitor
journey; CRM gets a moving lead pipeline board; automations get the
trigger/rule/action flow plus a run log that writes itself; app builds put
the phone screens first plus the need/screens/actions map; AI content gets a
brief-to-draft-to-approval stage. Every stage is a mechanism, never a metric.
New optional CMS fields on a project: `heroVideo` (external https URL; the
media library is images-only, 700KB), `tagline`, `gallery[]`,
`mobileGallery[]`, `serviceSlug`. Existing items keep working with all of
them empty (generic layout).

### F-40 - COPY V2 is written as CMS DRAFT, never straight to published
The client asked for the entire site copy to be rewritten in the sharper,
more energetic voice of a Gemini draft they supplied ("he actually wrote
good Hebrew"). Adopted: the energy, the `אתם` address, the concrete pains
(העתק-הדבק, לידים שנופלים בין הכיסאות), the punchier CTA ("בואו נעשה סדר
בעסק"). Rejected from that draft, because the truth rules win: invented
figures ("80% מהגולשים"), guarantees ("100% יציבה", "אפס לידים אבודים",
"בלי טעויות"), "הצוות שלי" (there is one founder, F-14), chatbots as a
service (F-2), and cliché phrases banned by COPY_STYLE.md. Delivery:
code defaults (`site.ts`, `content.ts`, home/about page strings) are updated,
and `migrations/0011_copy_v2_draft.sql` (generated by
`scripts/cms-copy-draft.mjs`) writes the same text into the CMS as DRAFT
for home/about/contact/faq/footer and all five services. Published copy is
untouched until the client reviews each page in preview and presses
Publish (R10, F-31). The FAQ item answers changed in `site.ts` are NOT
pushed to `faq_items` by this migration; they apply to fresh seeds only.
Strings that live in page code rather than the CMS (the home page's pain
